    ...
    return (
        <div className='mt-12 grid gap-y-8'>
            {products.map((product) => {
                ...
                return (
                    <Link to={`/products/${product.id}`} key={product.id} className='p-8 rounded-lg flex flex-col
                    sm:flex-row gap-y-4 flex-wrap bg-base-100 shadow-xl hover:shadow-2xl duration-300 group'>
                        ...
                        <p className='font-medium text-lg sm:ml-auto'>${price/100}</p>
                    </Link>
                )
            })}
        </div>
